...
创建 `.env` 文件（如果还没有）：
```env
DATABASE_URL=postgresql+asyncpg://root:123456@localhost:5432/doc_management
...
```bash
# 在项目根目录执行
pdm run python -m uvicorn src.main:app --reload --host 0.0.0.0 --port 8000
```
...
访问 http://localhost:8000/docs 查看 Swagger UI，可以：
- 查看所有可用的 API 端点
- 测试 API 调用
- 查看请求/响应格式
